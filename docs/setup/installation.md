...
This page is in transition during the public repo remodel.

Current rule:

- this repository is the public framework product
- it should not contain a live `WAI-Spoke/`
- you clone it, then bootstrap a spoke or a hub in a separate target project

The examples below still reflect some older path names and templates. Prefer the
new public surfaces when possible:

- `spoke/codebase/templates/spoke/`
- `spoke/codebase/templates/commands/`
- `hub/codebase/skills/`
- `hub/teachings/`
...
Clone the Wheelwright framework and keep that clone available as your local framework fork:
...
Copy spoke templates from the public spoke surface:

```bash
cp -r /path/to/framework/spoke/codebase/templates/spoke/. ./WAI-Spoke/
cp -r /path/to/framework/spoke/codebase/templates/commands ./WAI-Spoke/
```
...
```bash
cp -r /path/to/framework/hub/codebase/. ./
cp -r /path/to/framework/hub/teachings ./teachings
```

This creates:
- reusable hub skills
- hub advisor schemas
- hub teachings for distribution

It should not copy live runtime state from this framework repo.
...
- use the new `docs/`, `spoke/`, `shared/`, and `hub/` surfaces as the public map
- treat legacy `framework/docs/` and `framework/skills/` as compatibility copies until callers are updated
