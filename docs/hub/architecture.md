...
This page now describes the hub as a framework surface plus a separately bootstrapped runtime.
...
## Public Framework Structure

The public framework now separates reusable hub assets from live hub state:
...
Runtime files like ledgers, intake queues, machine records, and live registry state
belong in the bootstrapped hub project, not in the clean public framework export.
...
During the transition, legacy examples still exist under the top-level `hub/` tree.
Treat them as reference material, not as the final public contract.
