...
**Every session, check if the `.wwai/` folder has been updated since you last worked here:**
...
state = json.loads((wwai_path / "WWAI-State.json").read_text())
...
|------|---------|-------------|
| `WWAI-State.json` | Technical spec, foundation, session state | UPDATE |
| `WWAI-State.md` | Strategic context, vision | UPDATE |
| `wheel-signals.jsonl` | High-impact learnings | APPEND (never overwrite) |
| `kb-sync.json` | Hub sync status | READ ONLY |
| `WWAI-Guide.md` (this file) | Your instructions | READ ONLY |
...
state = json.loads(Path(".wwai/WWAI-State.json").read_text())
...
### 1. Add to decisions array in WWAI-State.json
```json
{
  "date": "2025-12-28",
...
### In WWAI-State.json `_project_foundation.evolution_log`:
```json
{
  "date": "2025-12-28",
  "change": "What changed",
...
### In WWAI-State.md Evolution Log table:
...
Check `wheelwright.framework_path` in WWAI-State.json, or:
1. Check if `wwai` command is in PATH
2. Look for `~/projects/wheelwright`
3. Look for `~/.wheelwright`
4. Ask user: "Where is your Wheelwright framework installed?"

### Finding the Hub
Check `wheelwright.hub_path` in WWAI-State.json, or:
1. Look for `~/wheelwright-hub`
2. Look for `~/.wheelwright-hub`
3. Ask user or create one: `cd <framework> && ./wwai hub create`

### Creating a Hub (if none exists)
```bash
cd <framework_path>
./wwai hub create --guided
```
...
./wwai hub status          # Check hub health
./wwai sync --all          # Sync all wheels
./wwai init <path>         # Add new wheel
...
### Commands for Users
```
wwai init [name]           # Initialize new wheel
wwai status                # Show wheel state summary
wwai spoke add [name]      # Add spoke to wheel
wwai spoke list            # List available spokes
wwai sync                  # Sync state files
wwai closeout              # Generate closeout files
wwai context               # Output context for LLM paste
wwai version               # Show version info
```
