...
- **Runs in:** Interactive user sessions AND as the nightly Gardener executor (both share this identity)
...
`ozi-nightly.md` (in `.claude/agents/`) is the **executor subagent** dispatched by Gardener for nightly automation. It runs Ozi's identity in a headless context. This file (`ozi.md`) defines the identity itself — shared by both interactive and nightly modes.
