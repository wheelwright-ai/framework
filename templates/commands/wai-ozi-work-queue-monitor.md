...
- Auto-dispatches to available builders
...
def auto_dispatch_ready_work(ready_lugs):
    ...
    for lug in ready_lugs:
        # Check if lug needs user approval first
        if lug['type'] == 'implementation' and not lug.get('user_approved'):
...
{if ready_work and auto_dispatch_enabled}
🚀 DISPATCHING NOW
  {list work being auto-assigned}
{/if}

{if ready_work and not auto_dispatch_enabled}
🆕 READY FOR WORK
  {list available work}
  💡 Tip: Enable auto-dispatch with:
