...
from .spoke_id import generate_spoke_id
...
def init_spoke(
    ...
    for template_file in template_files:
        ...
        if src.exists():
            ...
            content = content.replace('{{SPOKE_ID}}', spoke_id)
    ...
    try:
        ...
        if state_md.exists():
            ...
            vercel_detected = _detect_vercel_integration(spoke_path)
            if vercel_detected:
                snapshot.extend(["", "## Platform Detection", "", "**Vercel Integration Detected**", ""])
                snapshot.append("Consider implementing a keep-alive heartbeat to prevent cold starts:")
                snapshot.append("- Create `/api/health` endpoint")
                snapshot.append("- Configure Vercel Cron in `vercel.json` (*/5 * * * *)")
                snapshot.append("- Monitor with Vercel Analytics")
                snapshot.append("- See: https://vercel.com/docs/cron-jobs")
                if verbose:
                    print_info("  Detected Vercel - Added keep-alive recommendation")
...
def _detect_vercel_integration(spoke_path: Path) -> bool:
    """
    Detect if project uses Vercel platform.
    ...
    Returns:
        True if Vercel integration detected
    """
    # Check for vercel.json
    if (spoke_path / 'vercel.json').exists():
        return True
    
    # Check for .vercel directory
    if (spoke_path / '.vercel').exists():
        return True
    
    # Check package.json for vercel scripts
    package_json = spoke_path / 'package.json'
    if package_json.exists():
        try:
            data = json.loads(package_json.read_text())
            scripts = data.get('scripts', {})
            # Check for vercel-related scripts
            if any('vercel' in script.lower() for script in scripts.values()):
                return True
        except Exception:
            pass
