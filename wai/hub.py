...
class HubManager:
    ...
    def _score_candidate(self, candidate: HubCandidate) -> None:
        ...
        Scoring (v3.1+):
        - Has WAI-Spoke/: +15 (v3.1+ hub structure)
        - Has WAI-Spoke/WAI-State.json with is_hub=true: +20
        - Has hub-registry.json: +10
        - Has hub-profile.json: +10 (legacy v2.x)
        - Has registry/wheel-projects.json: +5 (legacy)
        - Has WAI-Spoke/ but no is_hub flag: -10 (likely a spoke)
        - Modified last 30 days: +2
        ...
        # Check for v3.1+ structure (WAI-Spoke/)
        wai_spoke_dir = path / 'WAI-Spoke'
        if wai_spoke_dir.exists():
            candidate.add_score(15, "Has WAI-Spoke/ (v3.1+ structure)")
            
            # Check if WAI-State.json marks this as a hub
            state_json = wai_spoke_dir / 'WAI-State.json'
            if state_json.exists():
                try:
                    import json
                    state_data = json.loads(state_json.read_text())
                    if state_data.get('wheelwright', {}).get('is_hub'):
                        candidate.add_score(20, "WAI-State.json marks is_hub=true")
                    else:
                        candidate.add_score(-10, "Has WAI-Spoke/ but not marked as hub (likely a spoke)")
                except Exception:
                    pass

        # Check for hub-registry.json (v3.1+)
        if (path / 'hub-registry.json').exists():
            candidate.add_score(10, "Has hub-registry.json")

        # Check for hub-profile.json (legacy v2.x)
        if (path / 'hub-profile.json').exists():
            candidate.add_score(10, "Has hub-profile.json (legacy)")

        # Check for registry/wheel-projects.json (legacy)
        if (path / 'registry' / 'wheel-projects.json').exists():
            candidate.add_score(5, "Has registry/wheel-projects.json")

        # Check modification time
        try:
            # Check modification time of hub files
            for hub_file in ['WAI-Spoke/WAI-State.json', 'hub-registry.json', 'hub-profile.json']:
    ...
    def verify_hub_structure(self, path: Path) -> Tuple[bool, Optional[str]]:
        """
        Verify valid hub structure (v3.1+ with WAI-Spoke).

        Required:
        - WAI-Spoke/ directory exists
        - WAI-Spoke/WAI-State.json exists
        - hub-registry.json exists (or legacy registry/wheel-projects.json)
        ...
        wai_spoke_dir = path / 'WAI-Spoke'
        if wai_spoke_dir.exists():
            # Verify WAI-State.json
            state_json = wai_spoke_dir / 'WAI-State.json'
            if not state_json.exists():
                return False, f"Missing WAI-Spoke/WAI-State.json at {path}"
            
            # Check for hub registry (new or legacy)
            hub_registry = path / 'hub-registry.json'
            legacy_registry = path / 'registry' / 'wheel-projects.json'
            if not (hub_registry.exists() or legacy_registry.exists()):
                return False, f"Missing hub-registry.json or registry/wheel-projects.json at {path}"
            
            return True, None

        # Legacy v2.x structure
        # Check hub-profile.json (legacy)
        hub_profile = path / 'hub-profile.json'
        if not hub_profile.exists():
            return False, f"Missing hub-profile.json or WAI-Spoke/ at {path}"
    ...
    def _create_hub_structure(self, hub_path: Path) -> None:
        ...
        Creates v3.1+ unified structure:
        - hub_path/WAI-Spoke/ (hub as a wheel)
        - hub_path/hub-registry.json (backwards compat)
        - hub_path/hub-security-policy.json
        - hub_path/hub-learning-index.md
        - hub_path/learnings/ (categories)
        - hub_path/registry/ (legacy)
        ...
        ensure_directory(hub_path / 'learnings')
        ensure_directory(hub_path / '.WAI-registry')

        # Get framework path to find templates
        framework_path = Path(__file__).parent.parent
        hub_templates = framework_path / 'templates' / 'HUB'
        
        # Create WAI-Spoke structure from templates
        wai_spoke_dir = hub_path / 'WAI-Spoke'
        ensure_directory(wai_spoke_dir)
        ensure_directory(wai_spoke_dir / 'seed' / 'ingest')
        ensure_directory(wai_spoke_dir / 'seed' / 'reference')
        ensure_directory(wai_spoke_dir / 'reference')
        ensure_directory(wai_spoke_dir / '_framework')

        # Copy WAI-Spoke templates
        wai_spoke_templates = hub_templates / 'WAI-Spoke'
        if wai_spoke_templates.exists():
            for item in wai_spoke_templates.iterdir():
                if item.is_file():
                    shutil.copy2(item, wai_spoke_dir / item.name)
                elif item.name == 'seed' and item.is_dir():
                    # Copy seed README
                    seed_readme = item / 'README.md'
        ...
        for template_name in ['hub-registry.json', 'hub-security-policy.json', 'hub-learning-index.md']:
            ...
            if template_file.exists():
                shutil.copy2(template_file, hub_path / template_name)

        # Create learning category files
        learning_categories = ['architecture', 'performance', 'testing', 'security', 'workflow', 'tools']
        for category in learning_categories:
            category_file = hub_path / 'learnings' / f'{category}.jsonl'
            if not category_file.exists():
                category_file.touch()

        # Update WAI-State.json with hub-specific info
        state_json_path = wai_spoke_dir / 'WAI-State.json'
        if state_json_path.exists():
            state_data = json.loads(state_json_path.read_text())
            state_data['_hub_profile']['hub_config']['created_at'] = datetime.now().isoformat()
            state_data['_hub_profile']['hub_config']['hub_path'] = str(hub_path)
            state_data['wheel']['created'] = datetime.now().isoformat()
            state_data['wheel']['last_updated'] = datetime.now().isoformat()
            state_json_path.write_text(json.dumps(state_data, indent=2, ensure_ascii=False) + '\n')

        # Create legacy wheel-projects.json for backwards compatibility
        registry = {
            "version": "3.0",
            "projects": [],
            "groups": {}
