...
class SpokeUpdateProcessor:
    ...
    def __init__(self, spoke_path: Path):
        ...
        # Hub detection: check for hub-specific files
        self.is_hub = self._detect_hub()

    def _detect_hub(self) -> bool:
        """Detect if this is a hub (not just a spoke)."""
        # Check for hub-specific files at spoke_path root
        hub_indicators = [
            self.spoke_path / 'hub-registry.json',
            self.spoke_path / 'learnings',
            self.spoke_path / '.WAI-registry'
        ]
        
        # Also check WAI-State.json for is_hub flag
        state_json = self.wai_spoke_dir / 'WAI-State.json'
        ...
        # Check for physical hub indicators
        return any(indicator.exists() for indicator in hub_indicators)
    ...
    def _ingest_files(self, files: List[Path]) -> (List[str], List[Dict[str, Any]], List[str]):
        ...
        for path in files:
            ...
            # Hub-specific file handling
            if self.is_hub and self._is_hub_file(path):
                hub_result = self._process_hub_file(path)
                if hub_result:
                    ingested.append(path.name)
                    notes.append(hub_result)
                    path.unlink()
                    continue
    ...
    def _is_hub_file(self, path: Path) -> bool:
        """Check if file is hub-specific (not spoke)."""
        hub_file_patterns = [
            'hub-registry.json',
        ...
        return path.name in hub_file_patterns
    
    def _process_hub_file(self, path: Path) -> Optional[Dict[str, Any]]:
        """Process hub-specific file from seed/ingest/."""
        import shutil
        
        try:
            # Copy hub files to spoke_path root (not WAI-Spoke/)
            dest = self.spoke_path / path.name
            
            # Special handling for certain files
            if path.name == 'hub-registry.json' and dest.exists():
                # Merge registry rather than replace
                self._merge_hub_registry(path, dest)
            else:
                # Replace other hub files
            ...
            return {
                ...
                "type": "hub_file",
                "bytes": path.stat().st_size,
                "applied_to": [str(dest.relative_to(self.spoke_path))]
            }
    ...
    def _merge_hub_registry(self, source: Path, dest: Path) -> None:
        """Merge hub registry updates without losing existing wheels."""
        import json
        ...
        try:
            ...
            # Preserve existing wheels but update structure
            if 'wheels' in existing and 'wheels' in new_data:
                new_data['wheels'] = existing['wheels']
            
            # Update version and metadata
            new_data['version'] = new_data.get('version', existing.get('version'))
            
            # Write merged result
            dest.write_text(json.dumps(new_data, indent=2, ensure_ascii=False) + '\n')
        except Exception:
            # On error, just copy the new file
            import shutil
