...
If any FAIL in `cc-hooks` category: attempt auto-remediation (create missing hook scripts, update settings.json). Re-run check. If still failing, log failures and continue with reduced scope.
