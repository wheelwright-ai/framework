...
- **Treating SIGNAL routing as skip:** Never skip a lug with `routed_to: SIGNAL`. SIGNAL means deliver to hub inbox, not skip. Hub is a spoke too — it receives work like any other spoke.
- **Hub implements hub-side features separately:** Wrong. Framework is the blueprint. Hub is just another spoke. Pattern: Framework designs → publishes teaching → Hub (and all spokes) adopt. If target_files are in `hub/`, framework still implements the teaching/schema that hub will receive and apply. Framework builds the pattern; hub receives it.
...
| `wai-tool-advisor.md` | Cross-tool config audit — Ozi runs proactively on drift or stale setup |
...
- `/wai-tool-advisor` — Cross-tool optimization audit
- `/wai-claude-maximizer` — Compatibility redirect to Tool Advisor
