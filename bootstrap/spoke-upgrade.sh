...
mkdir -p WAI-Spoke/lugs/incoming
mkdir -p WAI-Spoke/lugs/outgoing
...
hook_cmd = os.path.join(os.getcwd(), ".claude", "hooks", "user-prompt-submit.sh")
hook_entry_prompt = {
    "matcher": ".*",
...
| `lugs/incoming/` | Incoming items from hub/other projects |
| `lugs/outgoing/` | Outbound items for hub/other projects |
