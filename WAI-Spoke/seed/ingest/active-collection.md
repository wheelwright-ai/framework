...
- Portable session chaining across tools via predecessor metadata
...
If the user has pasted or loaded a prior WAI Track in the conversation, treat it as a predecessor and carry that forward into turn 1 using `session_metadata`.

Turn 1 should then include:
...
If there is no predecessor, either omit `session_metadata` entirely or set `has_predecessor:false`.
...
`session_metadata` appears on **turn 1 only** when predecessor context exists. Subsequent points should not repeat it.
...
If this session continued from a predecessor, the continuity note should explicitly say the exported file links forward from that prior track so another agent can keep the chain going.
