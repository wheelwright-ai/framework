...
**What:** High-impact insights from spokes (now delivered via lugs)  
**Format:** Lugs with category="learning" containing impact scores  
**Sharing Threshold:** Impact score >= 8/10  
**Extraction:** Hub LEARN function pulls learning lugs and appends to learnings/*.jsonl  
**Categories:**
- `architecture.jsonl` - Design patterns
- `performance.jsonl` - Optimization techniques
- `testing.jsonl` - Quality strategies
- `security.jsonl` - Best practices
- `workflow.jsonl` - Process improvements
- `tools.jsonl` - Tool recommendations
...
Hub teaches spokes whenever framework improves OR has pending lugs to deliver:
...
**Hub side** - What happens:
1. ✅ Scans framework templates
2. ✅ Reads hub templates
3. ✅ Generates upgrade-adoption-plan.json
4. ✅ Signs plan with hub fingerprint
5. ✅ **NEW: Route hub lugs** - Find all lugs in hub/WAI-Hub/WAI-Lugs.jsonl with `destination_wheel_id="<spoke-name>"`
6. ✅ **NEW: Append to spoke** - Add those lugs to spoke/WAI-Spoke/WAI-Lugs.jsonl
7. ✅ **NEW: Mark delivered** - Update status="delivered" in hub/WAI-Hub/WAI-Lugs.jsonl
8. ✅ Distributes to spoke/seed/ingest/ and hub/seed/ingest/

**Spoke side** (automatic):
1. Receives upgrade-adoption-plan.json
2. Verifies hub signature
3. Verifies file hashes
4. Proposes adoption (AI-guided)
5. Adopts selected files
6. **NEW: Processes new lugs** in spoke/WAI-Spoke/WAI-Lugs.jsonl by category
7. **NEW: Prepares response lugs** with destination_wheel_id="hub"
8. On next LEARN, sends learnings/feedback/tasks to hub

### Receiving Learnings (Via Lugs)

When spokes contribute high-impact learnings (triggered on `WAI hub reconcile` or closeout):

```python
# Hub LEARN function processes spoke lugs
# Pull from spoke/WAI-Spoke/WAI-Lugs.jsonl
spoke_lugs = [
    lug for lug in spoke_wailu gs.jsonl
    if lug["source_wheel_id"] == "<spoke>"
    and lug["destination_wheel_id"] == "hub"
...
for lug in spoke_lugs:
    if lug["category"] == "learning":
        ...
        if learning.get("impact_score", 0) >= 8:
            ...
            append_to_learning_file(
                f"learnings/{learning['category']}.jsonl",
                learning
            )
            # Update registry and index
            update_hub_registry(spoke_id, {"learnings_contributed": count})
...
# Append all processed lugs to hub store
append_to_jsonl("hub/WAI-Hub/WAI-Lugs.jsonl", processed_lugs)
```

**Hub processes lugs by category:**
- `learning` (≥8 impact) → learnings/*.jsonl
- `feedback` → hub/WAI-Hub/WAI-Lugs.jsonl
- `task` → hub/WAI-Hub/WAI-Lugs.jsonl (for hub to execute)
- `signal` → WAI-State.md log

**Hub Learning Index** shows:
- All learnings received (from learnings/*.jsonl)
- Organized by category
- Impact scores
- Wheel origins
- Applicable to other wheels

### Broadcasting Updates to All Spokes

When hub improves itself (e.g., new security policy):

```bash
# Create upgrade plan with hub files included
upgrade_plan = build_upgrade_plan(
    spoke_files=[...],
    hub_files=["hub-security-policy.json"],
    why_changed="Added fingerprint rotation"
)

# Sign with hub fingerprint
upgrade_plan = sign_upgrade_plan(
    upgrade_plan,
    hub_key="<hub-key>"
)

# Save and teach
save_upgrade_plan(upgrade_plan, "upgrade-adoption-plan.json")
./WAI teach --all-wheels
```

All spokes adopt same version, ensuring consistency.

---

## Decision Logic

### Should we teach this to spokes?

✅ **YES** if:
- Framework version bumped (even patch)
- Critical security update
- Major bug fix affecting multiple projects
- Architecture improvement with broad impact
- Breaking changes with migration path
- Hub has pending lugs to route to spokes

❌ **NO** if:
- Project-specific changes
- Experimental features not ready
- Changes incomplete in framework
...
Hub and Spoke structures with unified lug system:

```
Hub:
  hub/WAI-Hub/                    ← Hub's identity (parallel to spoke's WAI-Spoke/)
  ├── WAI-State.json              Hub configuration + analytics
  ├── WAI-State.md                Hub identity + operations
  ├── WAI-Guide.md                Hub assistant guide (generated)
  ├── WAI-Lugs.jsonl              Hub lugs: tasks, feedback, received learnings
  ├── WAI-File-Index.json         Hub file tr
