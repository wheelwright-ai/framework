...
def collect_expediter_stats() -> Dict[str, Any]:
    expediter_state_path = SPOKE_PATH / "advisors" / "expediter" / "scan_state.json"
...
def collect_tool_advisor_status() -> Dict[str, Any]:
    state_path = SPOKE_PATH / "advisors" / "tool-advisor" / "scan_state.json"
    if not state_path.exists():
        return {
            "audit_pending": True,
